...
async function preCadastro() {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      sucess.textContent =
        "Pré-Cadastro Realizado, aguarde aprovação de um administrador!";
...
async function trocarSenha() {
  ...
  const resq = document.getElementById("re-esq").value;
  ...
  try {
    const response = await fetch(
      "https://evoludesign.com.br/api-conipa/auth/recuperar-senha.php",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ re: resq, nova_senha: nova_senha }),
      }
    );
